...
// --------------------
const weekEvents = {
  1: ["401752825","401752822","401754525"],
  2: ["401752825","401752822","401754525","401754524","401754618","401757227",
      "401752689","401752937","401761603","401762461","401756883","401757228",
      "401757232","401752688","401752690","401752695","401752816","401752823",
      "401757229","401752948","401757230"]
};
...
function populateWeekDropdown() {
  ...
  Object.keys(weekEvents).forEach(weekNum => {
...
function savePicks(user, weekNum) {
  const form = document.getElementById('weekPicksForm');
...
async function loadGames(weekNum, user) {
  ...
  const eventIds = weekEvents[weekNum];
  if (!eventIds) return;
  ...
  try {
    ...
    const gameSlate = eventIds.map(id => data.events.find(ev => ev.id === id)).filter(Boolean);
...
function initPicks(user) {
  ...
  const weekSelect = document.getElementById('weekSelect');
  weekSelect.onchange = () => {
    loadGames(parseInt(weekSelect.value), user);
  };

  const initialWeek = Object.keys(weekEvents)[0];
  weekSelect.value = initialWeek;
  document.getElementById('welcomeMessage').innerText = `${user.displayName}'s Week ${initialWeek} Picks`;
  savePicks(user, initialWeek);
  loadGames(parseInt(initialWeek), user);
}

// --------------------
// Auth buttons & listener
// --------------------
